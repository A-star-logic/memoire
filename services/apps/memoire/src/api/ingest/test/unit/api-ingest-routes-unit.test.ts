...
vi.mock('@astarlogic/services-database/reporting');
const analyticsModule = await import('@astarlogic/services-database/reporting');

vi.mock('@astarlogic/services-parser', async () => {
  const actual = await vi.importActual<{ [key: string]: unknown }>(
    '@astarlogic/services-parser',
  );
  return {
    ...actual,
