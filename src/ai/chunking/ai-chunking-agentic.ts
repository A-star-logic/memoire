...
function isTooLarge({ text }: { text: string }): boolean {
  ...
  encoding.free();
  // gpt-35-turbo-16k models can generate upto 4096 tokens, so the document size should be less than that for full output
 ...
 * @param root.document document to get chunks
 * @param root.retryAttempt number representing the retry attempt
...
export async function createAgenticChunking({
  ...
  //fixed size splitting if the doc is gether than model desired size
  try {
    ...
    // 1-shot prompting on search chunk for agenting chunking, token size 618, info: https://smith.langchain.com/hub/wfh/proposal-indexing?organizationId=65e2223e-316a-5256-b012-5033801a97fa&tab=0
    ...
    if (retryAttempt < 4) {
      return createAgenticChunking({
        document,
        retryAttempt: retryAttempt + 1,
      });
    }
    throw new Error('All the 4 retry attempts to create agentic chunks failed');
  }
}
